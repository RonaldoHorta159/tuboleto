...
app.use(
  cors({
    origin:
      "https://kunturspirit.com/disponibilidad-de-boletos-para-machupicchu/", // Reemplaza con el dominio de tu página de WordPress
  })
);
...
app.get("/circuitos", async function (req, res) {
  try {
    const response = await fetch(
      "https://api-tuboleto.cultura.pe/visita/lugar-info?idLugar=llaqta_machupicchu"
    );
...
app.get("/fechas", async function (req, res) {
  try {
    ...
    const response = await fetch(
      `https://api-tuboleto.cultura.pe/visita/consulta-fechas-disponibles?nidruta=${nidruta}`
    );
...
app.get("/cupos", async function (req, res) {
  try {
    ...
    const response = await axios.post(
      `https://api-tuboleto.cultura.pe/visita/consulta-horarios`,
      {
        df_inicio,
        nidcircuito,
        nidlugar,
